...
~/usr/spark-3.5.0/work_dir/python_apps/example$ spark-submit \
--master local[2] \
--driver-memory 10g \
example.py \
1 49999 \
...
if __name__ == "__main__":
    if len(sys.argv) < 5:
        print("Usage error: example.py <min-range-Id> <max-range-Id> <path-input-log1> <path-input-log2> <path-output-log>")
        sys.exit(1)
    ...
    infoDF = spark.read.schema(infoSchema).option("header","true").option("delimiter", ";").csv(path_input_log1)
    ...
    songRDDFiltered = songRDD.filter(lambda record : record != songHeader).map(lambda line: line.split(";")).filter(
        lambda field: rangeId(int(field[1])
    )).map(
        lambda rec: Row(
        int(rec[1]),
    ...
    distSongDF = songDF.groupBy("userId").agg(countDistinct("songId")).withColumnRenamed("count(DISTINCT songId)", "distSongIds")

    genderSongDF = songDF.groupBy("userId", "genderId").count().withColumnRenamed("count", "topGenderIdSongs")

    genderWindow = Window.partitionBy("userId").orderBy(desc("topGenderIdSongs"))

    windowGenderSongDF = genderSongDF.withColumn("rank", row_number().over(genderWindow)).where("rank = 1").drop("rank"
