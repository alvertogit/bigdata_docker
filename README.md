...
- [Apache Spark] (4.1): an unified analytics engine for [Big Data] processing, with built-in modules for streaming, [SQL], [Machine Learning] and graph processing. It has high-level [API]s in [Scala] and [Python].
- [Hadoop] (3.5): an open-source software for reliable, scalable, distributed computing. It uses [Hadoop Distributed File System] ([HDFS]) which is suitable to work with large [RDD] (Resilient Distributed Datasets).
...
```bash
~/usr/spark-4.1.1/work_dir/scala_apps/example$ mvn package
```
...
```bash
~/usr/spark-4.1.1/work_dir/scala_apps/example$ spark-submit \
--master local[2] \
--class stubs.Example \
...
```bash
~/usr/spark-4.1.1/$ cd work_dir/python_apps/example
~/usr/spark-4.1.1/work_dir/python_apps/example$ python3 main.py 10000
```
...
```bash
~/usr/spark-4.1.1$ jupyter lab \
--notebook-dir=/usr/spark-4.1.1/work_dir/notebooks \
--ip='0.0.0.0' \
--port=8888 \
--no-browser \
--allow-root
